...
public class ShopInventory {
    ...
    public static double billItems(Item[] items, OnSale Sale) {
        ...
        if(Sale instanceof EasterSale){ 
            EasterSale Sale1 = (EasterSale) Sale;
            if(Sale1.saleCondition(items)){
                for(int i = 0; i < items.length; i++){
                    if(items[i] instanceof StorageCondition){ 
                        withStorageCondition(items[i], items[i].price);
                    }else{
                        WithoutStorageCondition(items[i], items[i].price );
                        }
            }}
            System.out.println("Actual Total: " + total );
            total = Sale1.reduction(total);
            Sale1.message();
        }else{
                for(int i = 0; i < items.length; i++){
                    if(items[i] instanceof StorageCondition){ 
    ...
    static public void WithoutStorageCondition(Item item , double price){
        System.out.println(item.toString() + " (" + price + ")");
    }

    static public void withStorageCondition( Item item, double price){
        StorageCondition x = (StorageCondition) item; 
        System.out.println(item.toString() + " (" + price + ") (Storage: " + x.storageProcedure() + ")");
    }
